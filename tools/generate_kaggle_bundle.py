...
def main():
    ...
    # 15. DEMO: Selective Full Fine-Tuning (GPT-2)
    notebook_cells.append(create_notebook_cell(
        "# DEMO: Selective Full Fine-Tuning\n"
        "# Trains the last decoder block's attention weights directly (no LoRA),\n"
        "# saves a delta checkpoint, merges it into a servable model, and restores it.",
        cell_type="markdown"
    ))
    notebook_cells.append(create_notebook_cell(
        "import torch\n"
        ...
        "print(f'Full-FT manager: {manager is not None}')\n"
        "print(f'Selected layers: {sorted(manager.selected.keys())}')\n"
        "print(f'Selected params: {manager.total_selected_params()} '\n"
        "      f'(~{manager.total_selected_params() * 4 / 1e6:.2f} MB fp32)')\n"
        "\n"
        ...
        "print(f'Loss before: {loss1:.4f}, after: {loss2:.4f}')\n"
        "\n"
        "# Save delta checkpoint and merge into a servable model\n"
        "ckpt = os.path.join(tmp, 'checkpoints', 'checkpoint-4')\n"
        "model.save_pretrained(ckpt)\n"
        "assert os.path.exists(os.path.join(ckpt, 'delta.safetensors'))\n"
        "assert os.path.exists(os.path.join(ckpt, 'delta.index.json'))\n"
        "print(f'Delta checkpoint saved: {os.path.basename(ckpt)}')\n"
        "with open(os.path.join(ckpt, 'delta.index.json')) as f:\n"
        "    idx = json.load(f)\n"
        "print(f'Delta tensors: {len(idx[\"weights\"])} keys '\n"
        ...
        "print(f'Merged servable model: {os.path.basename(merged_path)} '\n"
        "      f'({os.path.getsize(merged_path) / 1e6:.2f} MB)')\n"
        "\n"
        "# Restore from checkpoint (delta + optimizer state) and verify weights match\n"
        "restored = LemaModel.from_pretrained(ckpt)\n"
        "rm = restored.full_ft_manager\n"
        "all_close = all(\n"
        "    torch.allclose(manager.true_weights[k], rm.true_weights[k])\n"
        "    for k in manager.true_weights\n"
        ")\n"
        "print(f'Restored weights match trained weights: {all_close}')\n"
        "print('Selective full FT verification: OK')\n"
        "restored.close(); model.close()\n"
        "del restored, model, trainer, manager, rm\n"
        "import gc; gc.collect()\n"
        "if torch.cuda.is_available():\n"
        "    torch.cuda.empty_cache()"
    ))
