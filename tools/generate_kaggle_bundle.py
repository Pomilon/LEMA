...
def main():
    ...
    notebook_cells.append(create_notebook_cell(
        ...
        "q = run(8, 'int8-all')\n"
        "print(f'Loss gap vs baseline: {abs(q[0]-base[0]):.4f}')\n"
        "print(f'VRAM saved by int8-all: {base[2]-q[2]:.2f}GB '\n"
        "      f'({100*(1-q[2]/base[2]):.0f}% of baseline)')\n"
        "assert abs(q[0] - base[0]) < 0.5, 'quantized loss diverged from fp16 baseline'\n"
        "assert q[1] != q[0], 'quantized training did not advance'\n"
        "assert q[2] <= base[2] + 0.1, 'quantized training used more VRAM than baseline'\n"
