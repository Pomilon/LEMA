...
def main():
    ...
    notebook_cells.append(create_notebook_cell(
        ...
        "\n"
        "def run_w8a8(bits, label):\n"
        "    config = make_w8a8_config(bits)\n"
        "    model = LemaModel(config)\n"
        "    trainer = model.get_trainer()\n"
        ...
        "wbase = run_w8a8(None, 'fp16 baseline')\n"
        "wq8 = run_w8a8(8, 'int8-w8a8')\n"
        "assert wq8[1] != wq8[0], 'W8A8 training did not advance'\n"
        "assert abs(wq8[0] - wbase[0]) < 2.0, 'W8A8 loss diverged from fp16 baseline'\n"
        "speedup = wbase[3] / max(wq8[3], 1e-9)\n"
        "vram_delta = wbase[4] - wq8[4]\n"
        "rss_delta = wbase[5] - wq8[5]\n"
        "print(f'step2 time: fp16 {wbase[3]:.0f}ms -> int8 {wq8[3]:.0f}ms ({speedup:.2f}x)')\n"
        "print(f'peak VRAM: fp16 {wbase[4]:.2f}GB -> int8 {wq8[4]:.2f}GB ({vram_delta*1000:+.0f}MB)')\n"
        "print(f'peak RSS: fp16 {wbase[5]:.2f}GB -> int8 {wq8[5]:.2f}GB ({rss_delta*1000:+.0f}MB)')\n"
        "verdict = 'FASTER and CHEAPER' if (speedup > 1.0 and vram_delta > 0 and rss_delta > 0) else 'NOT yet faster-and-cheaper on this hardware'\n"
        "print(f'W8A8 verdict: {verdict}')\n"
        "print('=== W8A8 verification: OK ===')"
    ))
