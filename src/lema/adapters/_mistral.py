...
from transformers.models.mistral.modeling_mistral import (
    MistralDecoderLayer, MistralRMSNorm, MistralConfig, MistralRotaryEmbedding,
...
class MistralAdapter(LemaModelAdapter):
    ...
    def _construct_quantized_layer(self, layer_id: int, flat: torch.Tensor, full_ft_manager: Any = None) -> nn.Module:
        module = QuantizedMistralLayer(self.hf_config)
    ...
    def release_layer_module(self, module: nn.Module):
        if isinstance(module, QuantizedMistralLayer):
            return
    ...
    def forward_layer(self, layer_module: nn.Module, inputs: Any, **kwargs) -> Any:
        ...
        if isinstance(layer_module, QuantizedMistralLayer):
...
class QuantizedMistralLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self_attn = _QuantizedMistralAttention(config)
        self.mlp = _QuantizedMistralMLP(config)
...
class _QuantizedMistralAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class _QuantizedMistralMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
