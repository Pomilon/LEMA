...
Each stream kind can be stored quantized via the four config fields `weights_bits` (`None`/`0`/`8`/`4`), `opt_state_bits`, `grad_acc_bits`, and `kv_bits` (`None`/`0`/`8`; int4 is allowed for weights only). **Weights** are quantized and packed in the RAM staging buffer (`_pack_layer_to_ram`) and dequantized into the VRAM execution slot at consumption (`get_vram_flat_buffer`), so the disk and RAM footprint shrinks while compute runs at full precision — int8 ≈ 2×, int4 ≈ 4× on weight storage. **Full-FT optimizer states** and **gradient accumulators** are quantized in RAM (fp32 → int8, ≈ 4×) and dequantized only inside the per-layer AdamW step (`step_layer`); the disk accumulation backend stores int8 memmap sidecars with a float32 scale file, quantizing in place on reopen. **KV chunks** are quantized per chunk with a dynamic per-chunk scale (`KVChunkStore`, int8 ≈ 2×), wired through `generate_kv`. Precision is measured in `tests/test_quant_streaming.py`: int8 weights keep forward max-diff < 0.1 and train loss within 0.2 of the fp16 baseline; quantized full-FT tracks an fp32 reference within 5% relative weight error over 50 steps. The quantize/dequantize passes add a small per-step compute cost.
